...
def summarize_chat_history(persona, chat_history):
    """チャット履歴をLLMに要約させる関数"""
    global all_personas_summaries, total_input_chars, total_output_chars
    
    current_log = get_conversation_log(chat_history)
    summary_prompt = f"""
    あなたは優秀なアナリストです。以下のペルソナへのインタビュー履歴を読み、
    これまでの会話の重要なポイントを簡潔に、箇条書きで要約してください。
    この要約は、後続の質問の文脈として使われます。
    ...
    print_and_log(f"\n[中間要約作成中...] {persona.get('ペルソナ名')}さんのこれまでの会話を要約します。")
    summary = generate_text(summary_prompt, temperature=0.5)
    
    # 既存の要約を更新
    existing_summary = all_personas_summaries.get(persona.get('ペルソナ名'), "")
    if existing_summary:
        new_summary_prompt = f"これまでの要約と最新の要約を統合し、簡潔にまとめてください。\n\n---既存の要約---\n{existing_summary}\n\n---最新の要約---\n{summary}"
        integrated_summary = generate_text(new_summary_prompt, temperature=0.5)
        all_personas_summaries[persona.get('ペルソナ名')] = integrated_summary
    else:
        all_personas_summaries[persona.get('ペルソナ名')] = summary

    # 新しい要約をログに出力
    print_and_log(f"\n--- {persona.get('ペルソナ名')}さんへのインタビュー中間要約 ---")
    print_and_log(to_text(all_personas_summaries[persona.get('ペルソナ名')]))
    
    return all_personas_summaries[persona.get('ペルソナ名')]


def conduct_interview_flow(persona, chat_session, questions, topic, is_hypothesis_phase=False):
    """
    一問一答形式でインタビューを進め、各回答ごとに3回の更問を行う関数。
    5問ごとに会話履歴を要約し、プロンプトの肥大化を防ぎます。
    """
    global all_personas_full_history, all_personas_summaries
    ...
    # 要約システムを統合
    summary_interval = 5
    current_summary = all_personas_summaries.get(persona_name, "")
    
    for i, question in enumerate(questions):
        # 5問ごとに中間要約を作成し、チャット履歴をリセット
        if (i + 1) % summary_interval == 0 and i > 0:
            current_summary = summarize_chat_history(persona, chat_session.history)
            
            # 履歴を要約で置き換える
            chat_session.history = [
                {'role': 'user', 'parts': [f"これまでのインタビュー要約:\n{current_summary}"]},
                {'role': 'model', 'parts': ['承知しました。要約内容を基に、引き続きインタビューにお答えします。']}
            ]
            print_and_log(f"\n[セッション履歴リセット] 履歴を要約で置き換え、次の質問に進みます。")
    ...
    # 最終的な要約を生成して保存
    final_summary = summarize_chat_history(persona, all_personas_full_history[persona_name])
    all_personas_summaries[persona_name] = final_summary
...
def main():
    ...
    output_filename = os.path.join(log_dir, f"interview_report_step_by_step_sum_efi_{timestamp}.txt")
...
####「まねきねこ」は、あなたの“やりたい”が叶う、一番自由で身近な遊び場です。
        ...
        for persona in selected_personas:
            ...
            conduct_interview_flow(persona, chat, initial_questions, topic)
            
            # フェーズ間の要約を自動実行
            final_summary_of_phase1 = summarize_chat_history(persona, all_personas_full_history[persona_name])
            print_and_log(f"\n--- {persona_name}さんへのインタビュー要約 ---")
            print_and_log(to_text(final_summary_of_phase1))
            
            all_personas_summaries[persona_name] = final_summary_of_phase1 # 要約を保存
        ...
        **追加インタビュー質問**:
        ...
        for persona in selected_personas:
            ...
            conduct_interview_flow(persona, chat, extracted_new_questions, topic, 
