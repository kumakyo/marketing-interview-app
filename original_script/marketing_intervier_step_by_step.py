...
def parse_questions(question_text):
    ...
    questions_and_titles = re.findall(r'^[#|##]+(.+)', question_text, re.MULTILINE)
    return [q.strip() for q in questions_and_titles]

def conduct_interview_flow(persona_name, chat_session, initial_questions, topic, is_hypothesis_phase=False):
    """
    ...
    for i, question in enumerate(initial_questions):
        ...
        for follow_up_num in range(1, 4):
            current_conversation_log = "\n".join([f"{msg.role}: {part.text}" for msg in chat_session.history[-2:] for part in msg.parts if hasattr(part, 'text')])
...
"""
        ...
        **追加インタビュー質問**:
        ...
        for persona in selected_personas:
            ...
            conduct_interview_flow(persona_name, chat, extracted_new_questions, topic, is_hypothesis_phase=True)
